...
module Helpers = {
  let joinWith = (strings, separator) => {
    let rec run = (strings, acc) =>
      ...
      | list{} => acc
      | list{x} => acc ++ x
      | list{x, ...xs} => xs->run(acc ++ (x ++ separator))
      }
    strings->run("")
  }
}
...
module Grid = {
  ...
  module Repeat = {
    type rec t = [#repeat(value, list<trackList>)]
  ...
  module Template = {
    type rec t = [#list(list<value>) | #none]
    and value = [
      | LengthPercentage.t
      | Flex.t
      | MinMax.t
...
module BasicShape = {
  ...
  type t = [
    ...
    | #ellipse(ShapeRadius.t, ShapeRadius.t, option<(ShapePosition.t, ShapePosition.t)>)
    | #polygon(list<(LengthPercentage.t, LengthPercentage.t)>, option<FillRule.t>)
  ...
  let toString = (x: t) =>
    ...
    | #inset(top, right, bottom, left, borderRadius) =>
      ...
      let borderRadius =
        borderRadius
        ->Option.map(borderRadius => " round " ++ borderRadius->LengthPercentage.toString)
